...
int main(int argc, char** argv) {
    ...
    utilities::error_print("testing error print!\n");
    utilities::debug_print("testing debug print!\n");
    ...
    for(auto& sys: lif.systems) {
        std::cout << sys.name << ",\t\t";
        utilities::hex_print(sys.hex);
        std::cout << ",\ttype: ";
        utilities::hex_print(static_cast<uint8_t>(sys.type));
        std::cout << "\n";
    }
    ...
    uint8_t old_crc = config::spw::spw_calculate_crc_uint_F(old_segment);
    uint8_t new_crc = config::spw::spw_calculate_crc_uint_F(new_segment);

    std::cout << "old crc: ";
    utilities::hex_print(old_crc);
    std::cout << ", new crc: ";
    utilities::hex_print(new_crc);
    std::cout << "\n";

    std::vector<uint8_t> cmos2_train = deck.get_command_bytes_for_sys_for_code(0x0f, 0x1f);
    std::cout << "got cmos2 train command:\t";
    utilities::hex_print(cmos2_train);
    ...
    utilities::hex_print(template_cmd);
    std::cout << "got modified command:\t";
    utilities::hex_print(modified_cmd);
    ...
    std::vector bs = utilities::splat_to_nbytes(4,splat_val);
    for(auto& b: bs) {
        std::cout << (int)b << "\n";
    }

    uint32_t unsplat = utilities::unsplat_from_4bytes(bs);
    ...
    for(auto fragment: fr_res) {
        utilities::hex_print(fragment);
    }

    std::cout << "testing string to uint8_t...\n";
    std::vector<uint8_t> unpack1 = utilities::string_to_chars("0x022b0314");
    std::vector<uint8_t> unpack2 = utilities::string_to_chars("0x00");
    utilities::hex_print(unpack1);
    utilities::hex_print(unpack2);
    std::cout << "\n";
