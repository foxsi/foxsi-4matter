...
int main(int argc, char** argv) {
    ...
    std::cout << "making command...\n";
    Command set_standby = deck->get_command_for_sys_for_code(cdtede.hex, 0x0a);
    std::cout << "derefing ptr...\n";
    auto test = cdtede_manager;

    std::cout << "sending request...\n";
    std::vector<uint8_t> reply = machine->sync_send_command_to_system(*cdtede_manager, set_standby);
    std::cout << "sent.\n";
    ...
    if (reply.size() > 0) {
        ...
        std::cout << "trying to scan reply...\n";
        utilities::spw_print(reply, cdtede_manager->system.spacewire);
    } else {
        std::cout << "got no reply!\n";
    }
